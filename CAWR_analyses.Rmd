...
ids<-read_excel('data/cawr_otu_ids.xlsx', sheet=3)%>%
  mutate(SPECIES = ifelse(LEVEL != 'SPECIES' & !is.na(SPECIES), NA, SPECIES),
         ...
         LEVEL = case_when(FINAL_ID == GENUS ~ 'GENUS',
                           ...
                           TRUE ~ LEVEL))%>%
  mutate(ORDER = case_when(is.na(ORDER) ~ FINAL_ID, TRUE ~ ORDER), 
...
## what percent of each diet sample (otus) are 'diet'?
diet.perc<-otu.l%>%
  mutate(type=ifelse(ORDER %in% c('Araneae','Coleoptera','Blattodea','Diptera','Isopoda','Lepidoptera','Orthoptera'), 'prey', 'not'))%>%
...
ord.id<-good.id%>%
  ...
  summarize(OTU = length(unique(OTU)), n_fam= length(unique(na.omit(FAMILY))), n_sp = length(unique(na.omit(SPECIES))))%>%
  arrange(CLASS, ORDER)%>%
  left_join(id.ord)%>%
...
ord.id%>%ungroup()%>%dplyr::select(ORDER, samples, OTU, RRA, id_mean, otu_n_mean)%>%melt(id.vars='ORDER')%>%
  ...
  coord_flip()+
  scale_x_discrete(limits=rev(c('Diptera','Lepidoptera','Orthoptera','Araneae','Coleoptera','Isopoda','Blattodea','Siphonaptera','Psocoptera','Hymenoptera')))
...
nroc<-read_excel("data/NROC CACW Nestling Dev 2012-2013 for Kailen.xlsx", sheet=1)%>%
  dplyr::select(Site, Territory, nest=`Nest No`, dev_delay=`Ave 1 HD Diff Develop/Nest`, `Band Date`)%>%
...
write.csv(bird, 'manuscript/data/CAWR_bird_productivity.csv')
...
bird%>%
  dplyr::select(-egg1:-egg4, -Site)%>%
  pivot_longer(-ID:-egg_date)%>%
...
ggsave("/Users/collnell/Dropbox/Projects/CAWR/manuscript/figures/Fig_S3_birdvars.pdf", width=5, height=3.5)
...
# area of 100m radius circle = 31415.9
...
arth<-read.csv('CAWR_clean_cast.csv')%>%filter(!is.na(Diptera))%>% # drop plants that we not in block
  mutate(Territory = case_when(Location == 'SCR' & Territory == 2 ~ '5', TRUE ~ as.character(Territory)))%>% # to match to veg data
  mutate(ID = paste(Location, Territory, sep='_'))%>%dplyr::select(-Location, -Territory)
arth # 1373 rows
...
canopy.he<-arth.time.he%>%filter(sampling_method == 'canopy')%>%ungroup()%>%dplyr::select(-sampling_method)%>%
  separate(ID, into=c('site','terr'), sep='_') # 70 each
ground.he<-arth.time.he%>%filter(sampling_method == 'ground')%>%ungroup()%>%dplyr::select(-sampling_method)%>%
  separate(ID, into=c('site','terr'), sep='_')
...
adonis2(canopy.he%>%dplyr::select(-site, -terr, -he)%>%vegdist('bray')~he*terr, data=canopy.he, permiutaitons=1000)
# just habitat element

adonis2(ground.he%>%dplyr::select(-site, -terr, -he)%>%vegdist('bray')~he*terr, data=ground.he, permiutaitons=1000)
# he and terr
...
```{r}
## aggregate data at territory (ID) level
## bird reproductive data
bird

## vegetation cover
veg.cover

## territory-level estimates of arthropod biomass 
canopy.terr
ground.terr
...
terr.df<-read.csv('manuscript/CAWR_territory.csv')
